...
## This is a copy of the master README.md that resides in another repo.
The url for that master copy is: https://github.com/MichaelKentBurns/mcc
That is organized for VS Code, Terminal, and other IDEs.
That form has .c files in src/, .h files in includes/, etc. 

This repo has the same files, but organized flat because that for some reason is the way XCode prefers.
XCode is a great IDE, but does things it's own way. 
So, I'll try to keep these repos in sync but consider that other one the official copy.
It has all of the issues and comments.
